...
async function main() {
  ...
  for (const vp of voicePresetData) {
    await prisma.voicePreset.upsert({
      where: { ttsVoiceName_languageCode: { ttsVoiceName: vp.ttsVoiceName, languageCode: vp.languageCode } },
      update: { name: vp.name },
      create: vp,
    });
  }
  ...
  // ─── Prompt Templates ──────────────────────────────────────────────────
  const templateData = [
    { name: "Hand Hygiene Training", category: "infection_control", description: "WHO 5 moments of hand hygiene", promptTemplate: "Proper hand hygiene technique following WHO guidelines for healthcare workers in resource-limited settings", defaultConfig: { riskLevel: "L1", duration: 180 }, locale: "en" },
    { name: "Patient Assessment", category: "clinical_skills", description: "Systematic patient assessment approach", promptTemplate: "Systematic approach to patient assessment including vital signs, history taking, and physical examination for primary healthcare workers", defaultConfig: { riskLevel: "L2", duration: 300 }, locale: "en" },
    { name: "Emergency Triage", category: "emergency", description: "Emergency triage protocols", promptTemplate: "Emergency triage system for rural clinics, covering START triage method and critical decision-making in mass casualty scenarios", defaultConfig: { riskLevel: "L3", duration: 240 }, locale: "en" },
    { name: "Medication Administration", category: "medication", description: "Safe medication administration practices", promptTemplate: "Safe medication administration including the 5 rights of medication, common drug interactions, and adverse reaction recognition", defaultConfig: { riskLevel: "L2", duration: 240 }, locale: "en" },
    { name: "Wound Care Basics", category: "clinical_skills", description: "Basic wound care and dressing", promptTemplate: "Basic wound care and dressing techniques including wound assessment, cleaning, and appropriate dressing selection for different wound types", defaultConfig: { riskLevel: "L1", duration: 180 }, locale: "en" },
    { name: "PPE Donning and Doffing", category: "infection_control", description: "Personal Protective Equipment procedures", promptTemplate: "Step-by-step guide for donning and doffing Personal Protective Equipment (PPE) for infection prevention in healthcare settings", defaultConfig: { riskLevel: "L2", duration: 180 }, locale: "en" },
  ];
  // Use upsert-like logic: delete old templates then recreate
  await prisma.promptTemplate.deleteMany({ where: { isGlobal: true, name: { in: templateData.map(t => t.name) } } });
  for (const t of templateData) {
    await prisma.promptTemplate.create({ data: { ...t, isGlobal: true } });
  }
  ...
  console.log("   - 8 voice presets (ko, en, fr, es)");
  console.log("   - 6 prompt templates (clinical, emergency, infection control)");
}
