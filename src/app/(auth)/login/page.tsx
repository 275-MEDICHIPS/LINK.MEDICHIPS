"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import Image from "next/image";
import { useSearchParams } from "next/navigation";
import { useTranslations } from "next-intl";
import { useAuthStore } from "@/stores/auth-store";

export default function LoginPage() {
  const t = useTranslations("auth");
  const setUser = useAuthStore((s) => s.setUser);
  const searchParams = useSearchParams();
  const redirect = searchParams.get("redirect") || "/";
  const error = searchParams.get("error");
  const [loading, setLoading] = useState(false);

  // Pick up oauth_user cookie after Google callback redirect
  useEffect(() => {
    const cookieStr = document.cookie
      .split("; ")
      .find((c) => c.startsWith("oauth_user="));
    if (cookieStr) {
      try {
        const userData = JSON.parse(decodeURIComponent(cookieStr.split("=").slice(1).join("=")));
        setUser(userData);
        // Clear the cookie
        document.cookie = "oauth_user=; path=/; max-age=0";
        window.location.href = redirect;
      } catch {
        // ignore
      }
    }
  }, [setUser, redirect]);

  function handleGoogleLogin() {
    setLoading(true);
    const params = new URLSearchParams({ redirect });
    window.location.href = `/api/v1/auth/oauth/google?${params.toString()}`;
  }

  return (
    <div className="space-y-8">
      {/* Logo */}
      <div className="text-center">
        <Image src="/logo.png" alt="MEDICHIPS" width={56} height={56} className="mx-auto h-14 w-14" />
        <h1 className="mt-4 text-2xl font-bold text-gray-900">
          {t("welcomeTitle")}
        </h1>
        <p className="mt-1 text-sm text-gray-500">
          {t("welcomeSubtitle")}
        </p>
      </div>

      {/* Error */}
      {error && (
        <div className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">
          {error === "no_code" && "Google 인증에 실패했습니다. 다시 시도해주세요."}
          {error === "token_failed" && "Google 인증 토큰 오류가 발생했습니다."}
          {error === "userinfo_failed" && "Google 계정 정보를 불러올 수 없습니다."}
          {error === "oauth_failed" && "로그인에 실패했습니다. 다시 시도해주세요."}
        </div>
      )}

      {/* Google Login */}
      <button
        onClick={handleGoogleLogin}
        disabled={loading}
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-4 text-[15px] font-semibold text-gray-700 shadow-sm transition-all hover:bg-gray-50 hover:shadow-md disabled:opacity-50"
      >
        <svg className="h-5 w-5" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        {loading ? "로그인 중..." : "Google 계정으로 로그인"}
      </button>

      {/* Divider */}
      <div className="text-center text-xs text-gray-400">
        Google 계정으로 간편하게 시작하세요
      </div>

      {/* Footer link */}
      <div className="text-center text-sm text-gray-500">
        <Link href="/" className="font-medium text-brand-500 hover:text-brand-600">
          홈으로 돌아가기
        </Link>
      </div>
    </div>
  );
}
